#!/usr/bin/env python3
"""
Neutral Link Analysis Script

Analyzes multiple images from a work order to determine if a neutral link has been installed.
...
async def analyse_neutral_link_installation(image_folder: str) -> Dict[str, Any]:
    """Analyse a folder for neutral link installation using multi-image analysis with image grids."""
    ...
    analyzer = ImageAnalyzer(config_manager, "NeutralLinkInstallation")
    ...
    prompt_config = config_manager.get_prompt_config("NeutralLinkInstallation")
    ...
    result = {
        ...
        "neutral_link_installed": parsed_response.get("neutral_link_installed", False),
...
async def main():
    ...
    try:
        result_json = await analyse_neutral_link_installation(folder_path)
        print(json.dumps(result_json, indent=2))
    except Exception as e:
        logging.error(f"Neutral link installation analysis failed: {e}")
        raise
