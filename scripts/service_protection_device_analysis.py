#!/usr/bin/env python3
"""
Service Protection Device Analysis Script

Analyzes multiple images from a work order to determine if Service Protection Devices
or Meter Protection Devices have been added to the meter board.
Uses multi-image analysis to compare before and after images.
"""
...
async def analyse_service_protection_devices(image_folder: str) -> Dict[str, Any]:
    """Analyse a folder for service protection devices using multi-image analysis with image grids."""
    ...
    analyzer = ImageAnalyzer(config_manager, "ServiceProtectionDevices")
    ...
    prompt_config = config_manager.get_prompt_config("ServiceProtectionDevices")
    ...
    result = {
        ...
        "devices_added": parsed_response.get("devices_added", False),
...
async def main():
    ...
    default_dir = Path(__file__).parent.parent / "artefacts" / "SPD-MPD-Installation" / "Example 2"
    ...
    try:
        result_json = await analyse_service_protection_devices(folder_path)
        print(json.dumps(result_json, indent=2))
    except Exception as e:
        logging.error(f"Service protection device analysis failed: {e}")
        raise
